...
| src/lib/queries/admin.ts | `useAdminProofReviewQueue` | hook-branch | Returns `demoAdminProofQueue` instead of `GET /proofs?review_mode=review_queue`. | Admin proof review list | Demo items derived from `demoProofs`. |
