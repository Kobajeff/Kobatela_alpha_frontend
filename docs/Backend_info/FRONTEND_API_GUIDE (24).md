...
- `allowed_actions` is the backend source of truth for CTAs and always includes `VIEW_SUMMARY`, `VIEW_MILESTONES`, `VIEW_PROOFS`. Escrow-linked sender relations may additionally receive `ACTIVATE_ESCROW` (draft + milestones + valid fraud config), `FUND_ESCROW` (funding allowed by state machine), `EDIT_MILESTONES` (draft with no proofs/payments), `EDIT_FRAUD_CONFIG_ESCROW` (always for sender relation), `EDIT_FRAUD_CONFIG_MILESTONE` (open waiting milestone without proof), `MARK_DELIVERED` (escrow FUNDED), `CLIENT_APPROVE` (release condition satisfied), `CLIENT_REJECT` (non-terminal), `CHECK_DEADLINE` (FUNDED + past deadline), `DECIDE_PROOF` (pending proof with sender validator), and `REQUEST_ADVISOR_REVIEW` (any pending proof). Escrow-linked provider relations may receive `UPLOAD_PROOF_FILE` when escrow status is ACTIVE/FUNDED and a current milestone is submittable, and `SUBMIT_PROOF` when no active proof is present for that milestone. `OPS`/`UNKNOWN` relations keep only the view actions.【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1683-L1715】
...
- Proof AI scores (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`) are cleared for any non-OPS relation; `ai_summary_text` is exposed only for sender relations or ops viewers.  
- Proof metadata is stripped of payout rails and internal analytics for `SENDER`/`PROVIDER`/`PARTICIPANT` relations; advisors receive a partially filtered subset (no payout rails, limited analytics).  
...
| Method | Path | Summary | Scope | Request → Response | UI Label | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| POST | `/sender/payments/{payment_id}/execute` | Execute payout as sender | sender | — → `PaymentRead` | Requires escrow-linked sender relation; returns redacted `PaymentRead` if already executed. | 【F:app/routers/sender_payments.py†L17-L39】【F:app/services/payments.py†L324-L357】 |
