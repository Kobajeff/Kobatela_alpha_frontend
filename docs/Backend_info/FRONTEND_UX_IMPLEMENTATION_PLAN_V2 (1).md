...
### S2 Session
- **Role gating:** sender/provider/admin/advisor per endpoint scope; others blocked.【F:app/routers/auth.py†L179-L198】
- **Forbidden fields:** N/A.
...
### S3 Escrow List
- **Role gating:** sender/provider/support; advisor/admin not in scope.【F:app/routers/escrow.py†L33-L76】
- **Forbidden fields:** apply UI contract redaction rules for sender/provider/support views; advisor masking not applicable because scope excludes advisor.【F:docs/FRONTEND_UI_CONTRACT.md†L103-L110】【F:app/routers/escrow.py†L33-L76】
...
### S4 Escrow Detail
- **Role gating:** sender/provider/support/admin only; advisor blocked.【F:app/routers/escrow.py†L216-L230】
- **Forbidden fields:** redact sender/provider/support views per UI contract (amount/currency/beneficiary_id masking rules); advisor masking not applicable because scope excludes advisor.【F:docs/FRONTEND_UI_CONTRACT.md†L92-L101】【F:app/routers/escrow.py†L216-L230】
...
### S5 Escrow Summary
- **Role gating:** sender/provider only.【F:app/routers/escrow.py†L233-L249】
- **Forbidden fields:** mask proof metadata, PSP refs, idempotency keys per UI contract for sender/provider views; advisor masking not applicable because advisor scope is not allowed here.【F:docs/FRONTEND_UI_CONTRACT.md†L56-L69】【F:docs/FRONTEND_UI_CONTRACT.md†L146-L153】【F:app/routers/escrow.py†L233-L249】
...
### S7 Milestones
- **Role gating:** list access for sender/provider/admin/support; detail access for admin/sender/provider only.【F:app/routers/escrow.py†L271-L303】
- **Forbidden fields:** apply UI contract masking for sender/provider/support/admin views; advisor masking not applicable because scope excludes advisor.【F:docs/FRONTEND_UI_CONTRACT.md†L115-L120】【F:app/routers/escrow.py†L271-L303】
...
- **Role gating:** admin/support/sender (sender limited to own draft escrows); hide CTA for provider/advisor.【F:app/routers/escrow.py†L252-L268】【F:app/services/escrow.py†L965-L1050】
...
- **Advisor endpoints** are present (advisor assignment, review requests, advisor queues) and scoped to sender/advisor as defined in the API guide; ensure UI flows use these canonical routes rather than treating them as TODO.【F:docs/FRONTEND_API_GUIDE.md†L112-L140】【F:app/routers/advisors.py†L147-L296】
...
## Change Log
- **Substitutions**
  - BEFORE: “sender/provider/admin only per endpoint scope; others blocked.” (S2 Session role gating).  
    AFTER: “sender/provider/admin/advisor per endpoint scope; others blocked.”  
    Reason: `/auth/me` allows advisor scope.  
    Backend source: `app/routers/auth.py` lines 179-198.
  - BEFORE: “Advisor must not see amount/currency/beneficiary_id; apply UI contract redaction rules.” (S3 Escrow List forbidden fields).  
    AFTER: “apply UI contract redaction rules for sender/provider/support views; advisor masking not applicable because scope excludes advisor.”  
    Reason: `/escrows` excludes advisor scope.  
    Backend source: `app/routers/escrow.py` lines 33-76.
  - BEFORE: “redact advisor view of amount/currency/beneficiary_id; mask sensitive release_conditions_json content if present.” (S4 Escrow Detail forbidden fields).  
    AFTER: “redact sender/provider/support views per UI contract… advisor masking not applicable because scope excludes advisor.”  
    Reason: `/escrows/{id}` excludes advisor scope.  
    Backend source: `app/routers/escrow.py` lines 216-230.
  - BEFORE: “mask proof metadata… hide amounts for advisor if summary used in advisor context (should not be).” (S5 Escrow Summary forbidden fields).  
    AFTER: “mask… for sender/provider views; advisor masking not applicable because advisor scope is not allowed here.”  
    Reason: `/escrows/{id}/summary` limited to sender/provider scopes.  
    Backend source: `app/routers/escrow.py` lines 233-249.
  - BEFORE: “advisor must not see amount/currency; mask geofence details…” (S7 Milestones forbidden fields).  
    AFTER: “apply UI contract masking… advisor masking not applicable because scope excludes advisor.”  
    Reason: Milestone list/detail scopes omit advisor.  
    Backend source: `app/routers/escrow.py` lines 271-303.
  - BEFORE: “admin/support only; hide CTA for sender/provider/advisor.” (S14 Milestone Create role gating).  
    AFTER: “admin/support/sender (sender limited to own draft escrows); hide CTA for provider/advisor.”  
    Reason: `/escrows/{id}/milestones` allows sender with constraints.  
    Backend source: `app/routers/escrow.py` lines 252-268; `app/services/escrow.py` lines 965-1050.
  - BEFORE: “Advisor endpoints… do not appear in FRONTEND_API_GUIDE.md… treat as TODO.” (Risks section).  
    AFTER: “Advisor endpoints are present… ensure UI flows use these canonical routes rather than treating them as TODO.”  
    Reason: Advisor routes exist and are documented.  
    Backend source: `docs/FRONTEND_API_GUIDE.md` lines 112-140; `app/routers/advisors.py` lines 147-296.
- **Additions**
  - None.
- **Deletions**
  - None.
