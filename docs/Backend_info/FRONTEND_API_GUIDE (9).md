...
```
`ExternalProofUploadResponse` example. 【F:app/schemas/external_proofs.py†L35-L55】
...
```
`ExternalProofSubmit` and `ExternalProofSubmitResponse` examples. 【F:app/schemas/external_proofs.py†L58-L120】
...
```
`ExternalEscrowSummary` example. 【F:app/schemas/external_proofs.py†L123-L158】
...
- Preferred: `Authorization: Bearer <external_token>` header.
- Optional fallback: `X-External-Token` header.
- Backward-compatible: `?token=<external_token>` query param (avoid in UI due to leak risk).【F:app/security/external_tokens.py†L1-L31】

| Method | Path | Summary | Scope | UI Label | Evidence |
| --- | --- | --- | --- | --- | --- |
| POST | `/external/proofs/tokens` | Issue external proof token | sender/support/admin | UI — Sender/Admin (token issuance) | 【F:app/routers/external_proofs.py†L50-L78】 |
| POST | `/external/tokens/beneficiary` | Issue beneficiary token | sender/support/admin | UI — Sender/Admin (token issuance) | 【F:app/routers/external_proofs.py†L81-L109】 |
| POST | `/external/files/proofs` | Upload external proof file | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L138-L194】 |
| POST | `/external/proofs/submit` | Submit external proof | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L196-L315】 |
| GET | `/external/escrows/{escrow_id}` | External escrow summary | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L318-L368】 |
