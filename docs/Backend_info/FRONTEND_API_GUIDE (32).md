...
- `payment_mode` (string enum `"MILESTONE" | "DIRECT_PAY" | "NO_MILESTONE_CONDITIONAL"`, default `"MILESTONE"`)  
- `payout_destination_type` (string enum `"BENEFICIARY_PROVIDER" | "MERCHANT"`, nullable; defaults to `"MERCHANT"` for Direct Pay and `"BENEFICIARY_PROVIDER"` otherwise)  
...
- `payout_destination_type` defaults to `"MERCHANT"` for Direct Pay escrows.
...
- Set `payment_mode` to `"NO_MILESTONE_CONDITIONAL"` to create a no-milestone escrow that still routes payouts to the provider/beneficiary.
- No-milestone conditional escrows must **not** include milestones in `release_conditions` or via milestone creation endpoints.
- `payout_destination_type` defaults to `"BENEFICIARY_PROVIDER"` (merchant destinations remain Direct Pay only).
