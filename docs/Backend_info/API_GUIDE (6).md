...
Method | Path | Handler | Auth | Required scope(s) | Request model | Response model | Success codes | Common error codes | Evidence
---|---|---|---|---|---|---|---|---|---
POST | /escrows/{escrow_id}/milestones | `create_milestone_for_escrow` | API key | admin, support | `MilestoneCreate` (escrow schema) | `MilestoneRead` (escrow schema) | 201 | INSUFFICIENT_SCOPE, NO_API_KEY, UNAUTHORIZED | 【F:app/routers/escrow.py†L252-L268】【F:app/schemas/escrow.py†L136-L171】【F:app/security/__init__.py†L33-L153】
...
POST | /external/files/proofs | `upload_external_proof_file` | Public (token) | — | multipart/form-data + `token` query | `ExternalProofUploadResponse` | 201 | UNSUPPORTED_FILE_TYPE, FILE_TOO_LARGE, FILE_UPLOAD_FAILED | 【F:app/routers/external_proofs.py†L138-L193】【F:app/schemas/external_proofs.py†L34-L44】
POST | /external/proofs/submit | `submit_external_proof` | Public (token) | — | `ExternalProofSubmit` + `token` query | `ExternalProofSubmitResponse` | 201 | TOKEN_ESCROW_MISMATCH, TOKEN_MILESTONE_MISMATCH, FILE_REQUIRED, FILE_METADATA_REQUIRED | 【F:app/routers/external_proofs.py†L196-L323】【F:app/schemas/external_proofs.py†L46-L67】
GET | /external/escrows/{escrow_id} | `get_external_escrow_summary` | Public (token) | — | Query: `token` | `ExternalEscrowSummary` | 200 | TOKEN_ESCROW_MISMATCH | 【F:app/routers/external_proofs.py†L326-L370】【F:app/schemas/external_proofs.py†L69-L84】
...
### External proofs
- **ExternalProofTokenRequest/Response**: escrow_id, milestone_idx, token, expires_at.【F:app/schemas/external_proofs.py†L13-L28】
- **ExternalProofUploadResponse**: file metadata for external uploads.【F:app/schemas/external_proofs.py†L34-L44】
- **ExternalProofSubmit/Response**: proof submission via token.【F:app/schemas/external_proofs.py†L46-L67】
- **ExternalEscrowSummary**: escrow/milestone summary for external view.【F:app/schemas/external_proofs.py†L69-L84】
