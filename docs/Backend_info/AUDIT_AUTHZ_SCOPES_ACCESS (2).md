...
| Enum | Values | Normalization | Evidence |
| --- | --- | --- | --- |
| `ApiScope` | `sender`, `provider`, `support`, `admin`, `advisor`, `user`, `pricing_admin`, `risk_admin` | Enum defines uppercase canonical names + lower-case aliases | `app/models/api_key.py` |
...
Users may also carry **capabilities** (e.g., `sender`, `provider`, `pricing_admin`, `risk_admin`) in `user.capabilities`. These are **not roles**, and they are **not sufficient authorization**; they are inputs to relational checks (e.g., escrow initiation/participation) alongside ownership/participant relationships.
...
| POST | `/proofs/{proof_id}/decision` | `decide_proof` | API key (user-linked) | any scope (user/admin/support/advisor) | 401/403/404 |
...
### `/sender/escrows` (app/routers/sender_escrow_milestones.py, sender_fraud_config.py)
| Method | Path | Handler | Auth | Scopes | Failure behavior |
| --- | --- | --- | --- | --- | --- |
...
| GET | `/sender/escrows/{escrow_id}/fraud-config` | `get_escrow_fraud_config` | API key (user-linked) | any scope (user/admin/support/advisor) | 401/403/404 |
| PATCH | `/sender/escrows/{escrow_id}/fraud-config` | `patch_escrow_fraud_config` | API key (user-linked) | any scope (user/admin/support/advisor) | 401/403/404 |
| GET | `/sender/escrows/{escrow_id}/milestones/{milestone_idx}/fraud-config` | `get_milestone_fraud_config` | API key (user-linked) | any scope (user/admin/support/advisor) | 401/403/404 |
| PATCH | `/sender/escrows/{escrow_id}/milestones/{milestone_idx}/fraud-config` | `patch_milestone_fraud_config` | API key (user-linked) | any scope (user/admin/support/advisor) | 401/403/404 |
