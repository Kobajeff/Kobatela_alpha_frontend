...
| POST | `/external/proofs/submit` | Submit external proof (escrow_id/milestone_idx optional; token supplies defaults) | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L196-L315】 |
| GET | `/external/escrows/summary` | External escrow summary (token-resolved) | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L318-L352】 |
| GET | `/external/escrows/{escrow_id}` | External escrow summary (path fallback) | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L355-L389】 |
| GET | `/external/proofs/{proof_id}/status` | Poll proof status (terminal when APPROVED/REJECTED) | token | UI — Beneficiary Portal | 【F:app/routers/external_proofs.py†L392-L430】 |
