...
POST | /external/proofs/tokens | `issue_external_proof_token` | API key | sender, support, admin | `ExternalProofTokenRequest` | `ExternalProofTokenResponse` | 201 | INSUFFICIENT_SCOPE, NO_API_KEY, UNAUTHORIZED | 【F:app/routers/external_proofs.py†L50-L78】【F:app/schemas/external_proofs.py†L13-L28】【F:app/security/__init__.py†L33-L153】
POST | /external/tokens/beneficiary | `issue_external_beneficiary_token` | API key | sender, support, admin | `ExternalBeneficiaryTokenRequest` | `ExternalProofTokenResponse` | 201 | INSUFFICIENT_SCOPE, NO_API_KEY, UNAUTHORIZED | 【F:app/routers/external_proofs.py†L81-L109】【F:app/schemas/external_proofs.py†L30-L32】
POST | /external/files/proofs | `upload_external_proof_file` | Public (token) | — | multipart/form-data + token via `Authorization: Bearer <token>` (preferred), `X-External-Token`, or legacy `?token=` | `ExternalProofUploadResponse` | 201 | UNSUPPORTED_FILE_TYPE, FILE_TOO_LARGE, FILE_UPLOAD_FAILED | 【F:app/routers/external_proofs.py†L138-L194】【F:app/security/external_tokens.py†L1-L31】【F:app/schemas/external_proofs.py†L34-L55】
POST | /external/proofs/submit | `submit_external_proof` | Public (token) | — | `ExternalProofSubmit` + token via `Authorization: Bearer <token>` (preferred), `X-External-Token`, or legacy `?token=` | `ExternalProofSubmitResponse` | 201 | TOKEN_ESCROW_MISMATCH, TOKEN_MILESTONE_MISMATCH, FILE_METADATA_REQUIRED | 【F:app/routers/external_proofs.py†L196-L315】【F:app/security/external_tokens.py†L1-L31】【F:app/schemas/external_proofs.py†L57-L99】
GET | /external/escrows/{escrow_id} | `get_external_escrow_summary` | Public (token) | — | Token via `Authorization: Bearer <token>` (preferred), `X-External-Token`, or legacy `?token=` | `ExternalEscrowSummary` | 200 | TOKEN_ESCROW_MISMATCH | 【F:app/routers/external_proofs.py†L318-L368】【F:app/security/external_tokens.py†L1-L31】【F:app/schemas/external_proofs.py†L102-L131】
...
Method | Path | Handler | Auth | Required scope(s) | Request model | Response model | Success codes | Common error codes | Evidence
---|---|---|---|---|---|---|---|---|---
GET | /admin/fraud/score_comparison | `fraud_score_comparison` | API key | admin, support | Query: proof_id | `dict` | 200 | PROOF_NOT_FOUND | 【F:app/routers/admin_tools.py†L110-L153】
GET | /admin/risk-snapshots | `list_risk_snapshots` | API key | admin, support | Query: subject_type, subject_id, limit | `list[RiskFeatureSnapshotRead]` | 200 | — | 【F:app/routers/admin_tools.py†L156-L179】【F:app/schemas/risk_snapshot.py†L8-L49】

**Internal processing (not directly exposed)**
- Risk features are derived from proofs and stored as snapshots; the only exposed API surface is via the admin/support risk snapshot listing above (no public endpoints found).【F:app/routers/admin_tools.py†L156-L179】
...
GET | /admin/proofs/review-queue | `get_proof_review_queue` | API key | admin, support | Query (filters, limit, offset) | `list[AdminProofReviewItem]` | 200 | INSUFFICIENT_SCOPE | 【F:app/routers/admin_tools.py†L39-L107】【F:app/schemas/proof.py†L159-L190】
...
GET | /sender/dashboard | `get_sender_dashboard` | API key | sender | Query: limit | `SenderDashboard` | 200 | USER_NOT_FOUND, INSUFFICIENT_SCOPE | 【F:app/routers/sender_dashboard.py†L29-L49】【F:app/schemas/dashboard.py†L11-L35】
...
- Preferred: `Authorization: Bearer <external_token>` header; optional `X-External-Token` header.
- Backward-compatible: `?token=<external_token>` query param (avoid for UI due to leak risk).【F:app/security/external_tokens.py†L1-L31】

**Schemas**
- **ExternalProofTokenRequest/Response**: escrow_id, milestone_idx, token, expires_at.【F:app/schemas/external_proofs.py†L13-L32】
- **ExternalProofUploadResponse**: storage metadata for external uploads.【F:app/schemas/external_proofs.py†L35-L55】
- **ExternalProofSubmit** (request): escrow_id, milestone_idx, type, storage_key/storage_url/sha256, metadata (optional).【F:app/schemas/external_proofs.py†L58-L99】
- **ExternalProofSubmitResponse**: proof_id, status, escrow_id, milestone_idx, created_at.【F:app/schemas/external_proofs.py†L102-L120】
- **ExternalEscrowSummary**: escrow status + milestone summaries without PII.【F:app/schemas/external_proofs.py†L123-L158】
...
- Standard payload uses `{"error": {"code", "message", ...}}` with optional `context`, `details`, and `field_errors` for backward compatibility.【F:app/utils/errors.py†L17-L44】
- HTTPException and validation errors are wrapped into the same payload shape by global exception handlers; validation errors map to `VALIDATION_ERROR` with per-field messages.【F:app/main.py†L222-L284】
...
- Error payloads retain backward-compatible `context` and `field_errors` fields to avoid breaking clients.【F:app/utils/errors.py†L25-L44】
