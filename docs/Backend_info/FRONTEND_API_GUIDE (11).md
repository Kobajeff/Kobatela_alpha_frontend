...
| --- | --- | --- | --- | --- | --- |
| POST | `/external/proofs/tokens` | API key (sender/support/admin) | `ExternalProofTokenRequest` → `ExternalProofTokenResponse` | Binds to escrow + milestone + beneficiary; expiry bounds enforced (422 when too short/long). | 【F:app/routers/external_proofs.py†L50-L109】【F:app/services/external_proof_tokens.py†L40-L118】 |
