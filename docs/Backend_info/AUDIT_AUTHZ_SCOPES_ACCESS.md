...
- **Auth required:** `require_scope({sender, provider, admin})`
...
| GET | `/auth/me` | `auth_me` | API key | sender/provider/admin | 401/403/404 for user missing |
...
- **Advisor scope**: `/auth/me` only accepts sender/provider/admin today; advisor has dedicated `/advisor/*` endpoints.
