...
- `admin_dashboard`, `admin_senders`, and `sender_dashboard` router modules exist but are **not mounted** in `get_api_router()` or `app.main`; their endpoints are not exposed until explicitly included.【F:app/routers/admin_dashboard.py†L11-L28】【F:app/routers/admin_senders.py†L17-L62】【F:app/routers/sender_dashboard.py†L1-L42】【F:app/routers/__init__.py†L33-L63】【F:app/main.py†L214-L219】
...
## 3. Contextual Role Enforcement (global rules)
- Escrow access is relationship-based (R6 RAL): access checks resolve escrow viewer context from `sender_user_id`/`provider_user_id` (plus participants) and enforce relation-specific errors (`NOT_ESCROW_SENDER`, `NOT_ESCROW_PROVIDER`, `NOT_ESCROW_PARTICIPANT`) when the caller lacks the required escrow relationship; RAL fallbacks use the same relation context.【F:app/security/context.py†L67-L150】【F:app/security/authz_rel.py†L39-L122】【F:app/security/ral.py†L71-L182】【F:app/errors/domain_errors.py†L8-L38】
...
- Relation-based ABAC context is computed from escrow linkage (`sender_user_id`, `provider_user_id`, optional participants) and ops/advisor flags; escrow summaries expose `viewer_context.relation` (`SENDER`/`PROVIDER`/`PARTICIPANT`/`OPS`/`UNKNOWN`) and `allowed_actions` derived from escrow/milestone/proof state for relationship-driven UI decisions.【F:app/security/authz_rel.py†L39-L122】【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1837】
...
PATCH | /escrows/{escrow_id}/merchant-selection | `update_merchant_selection` | API key | sender | `EscrowMerchantSelectionUpdate` | `EscrowSummaryRead` | 200 | INSUFFICIENT_SCOPE, NO_API_KEY, UNAUTHORIZED | 【F:app/routers/escrow.py†L216-L235】【F:app/schemas/escrow.py†L143-L163】【F:app/security/__init__.py†L33-L153】
...
##### Canonical v1 vs Legacy (compatibility)
- **Canonical v1 (recommended)**: opt-in with `canonical_version="v1"` *and* the `KOB_CANONICAL_ESCROW_V1` feature flag. Canonical v1 sets `canonical_v1=true`, requires `proof_submitter` + `payout_destination`, and derives routing from PartyRef + milestone presence (legacy `payment_mode`/`payout_destination_type` inputs are ignored when PartyRef is provided).  
  NOTE: Code reference: `app/services/escrow.py:create_escrow`, `app/services/feature_flags.py:is_canonical_escrow_v1_enabled`, `app/utils/escrow_routing.py:derive_payment_mode`.
- **Legacy (compat-only)**: `canonical_version` omitted. Legacy fields remain accepted for compatibility; PartyRef is optional unless `KOB_ENFORCE_PARTYREF_ON_LEGACY_CREATE=1`. When PartyRef is provided, it overrides legacy payout hints and routing is derived from PartyRef + milestones.  
  NOTE: Code reference: `app/services/escrow.py:create_escrow`, `app/services/feature_flags.py:is_partyref_required_on_legacy_create`, `app/utils/escrow_routing.py:payout_destination_type_for_escrow`.

##### PartyRef model (canonical party identity)
PartyRef is the canonical identity object for both proof submitter and payout destination. Validation rules include:
- `ON_PLATFORM` requires `user_id`, except merchants may supply `registry_id` instead.
- `OFF_PLATFORM` forbids `user_id` and requires `display_name` + `country`; non-merchant parties require `external_profile_id` (unless `registry_id` is provided).
- Beneficiaries cannot carry `registry_id`/`pricing_profile_id`.
NOTE: Code reference: `app/schemas/party_ref.py:PartyRef._validate_party_ref`.

##### Proof invariants (PR1/PR2/PR3 = safety-first)
- **PR1**: every milestone must have ≥1 proof request.  
- **PR2**: if there are no milestones, the escrow must have ≥1 escrow-level proof request.  
- **PR3**: releases require approved proofs; canonical v1 cannot bypass proofs via `release_conditions.requires_proof=false`.  
Canonical v1 always enforces PR1/PR2/PR3. Legacy escrows enforce PR1/PR2/PR3 when `KOB_ENFORCE_PROOF_COHERENCE=1` or `KOB_ENFORCE_GLOBAL_PROOF_GATING=1`.  
NOTE: Code reference: `app/services/proof_coherence.py:validate_proof_coherence`, `app/services/proof_coherence.py:ensure_proof_coherence`, `app/services/escrow.py:_ensure_release_conditions`, `app/protocol/policies/escrow_allowed_actions.py:compute_allowed_actions`, `app/services/payments.py:_assert_payout_preconditions`.

##### Milestone invariants (exact-slice + sum)
- **Exact-slice**: a milestone release amount must equal the milestone amount (no partial slice releases).  
  NOTE: Code reference: `app/use_cases/protocol/release_funds.py:enforce_milestone_amount_exact`, `app/services/payments.py:_assert_payout_preconditions`.
- **Sum of slices = escrow total** (with currency match): enforced for canonical v1 and for legacy when `KOB_ENFORCE_MILESTONE_SUM_STRICT=1`. Legacy without the flag logs a warning instead of blocking.  
  NOTE: Code reference: `app/services/escrow.py:_ensure_activation_allowed`, `app/services/escrow.py:_validate_canonical_milestone_totals`, `app/services/escrow.py:_log_milestone_sum_mismatch`, `app/services/feature_flags.py:is_milestone_sum_strict_enforced`.

##### Release-sensitive endpoints (not a force payout hammer)
- `POST /payments/execute/{payment_id}` (admin/support) and `POST /sender/payments/{payment_id}/execute` (sender) enforce proof coherence, proof gating, milestone exactness, and escrow funding state. These endpoints can reject execution when policy invariants are not satisfied.  
  NOTE: Code reference: `app/routers/payments.py:execute_payment`, `app/routers/sender_payments.py:execute_sender_payment`, `app/services/payments.py:execute_payment_release`, `app/services/payments.py:_assert_payout_preconditions`.

##### Feature flags (behavior in production)
- `KOB_CANONICAL_ESCROW_V1` — enables canonical v1 opt-in via `canonical_version="v1"`.  
  NOTE: Code reference: `app/services/feature_flags.py:is_canonical_escrow_v1_enabled`, `app/services/escrow.py:create_escrow`.
- `KOB_ENFORCE_PARTYREF_ON_LEGACY_CREATE` — requires PartyRef on legacy escrow creation.  
  NOTE: Code reference: `app/services/feature_flags.py:is_partyref_required_on_legacy_create`, `app/services/escrow.py:create_escrow`.
- `KOB_PARTYREF_READ_REPAIR` — writes missing PartyRef JSON fields on read (legacy compatibility).  
  NOTE: Code reference: `app/services/feature_flags.py:is_partyref_read_repair_enabled`, `app/services/escrow.py:_maybe_read_repair_party_refs`.
- `KOB_ENFORCE_PROOF_COHERENCE` — enforces PR1/PR2/PR3 for legacy escrows.  
  NOTE: Code reference: `app/services/feature_flags.py:is_proof_coherence_enforced`, `app/services/proof_coherence.py:ensure_proof_coherence`.
- `KOB_ENFORCE_GLOBAL_PROOF_GATING` — enforces proof gating for legacy escrows when milestones or proof requests exist (affects allowed actions and release gating).  
  NOTE: Code reference: `app/services/feature_flags.py:is_global_proof_gating_enforced`, `app/services/escrow.py:_ensure_release_conditions`, `app/protocol/policies/escrow_allowed_actions.py:compute_allowed_actions`.
- `KOB_ENFORCE_MILESTONE_SUM_STRICT` — enforces milestone totals against escrow amount for legacy escrows.  
  NOTE: Code reference: `app/services/feature_flags.py:is_milestone_sum_strict_enforced`, `app/services/escrow.py:_validate_canonical_milestone_totals`.
- `KOB_ALLOW_MERCHANT_MILESTONES` — allows merchant payouts with milestones for canonical v1 escrows (legacy remains blocked).  
  NOTE: Code reference: `app/services/feature_flags.py:allow_merchant_milestones`, `app/services/escrow.py:_ensure_activation_allowed`.
...
- `payment_mode` (string enum, deprecated) — legacy UI template; backend routing now uses milestones + PartyRef (read-only for canonical v1).  
- `payout_destination_type` (string enum, nullable) — legacy payout hint (`BENEFICIARY_PROVIDER` or `MERCHANT`), derived from `payout_destination` when present (read-only for canonical v1).  
- `proof_submitter` (object, nullable) — PartyRef describing who must submit proofs.  
- `payout_destination` (object, nullable) — PartyRef describing who receives payout.  
- `canonical_version` (string, nullable) — opt-in `"v1"` to enforce canonical PartyRef behavior when feature flag is enabled.  

Canonical v1 escrows enforce milestone totals and currency at activation when milestones exist:
- Sum of milestone `amount` values **must equal** `amount_total` (`ESCROW_MILESTONES_SUM_MISMATCH`).  
- Milestone `currency` values **must match** escrow currency (`ESCROW_MILESTONES_CURRENCY_MISMATCH`).  
Legacy escrows keep the warning-only behavior for sums below total unless `KOB_ENFORCE_MILESTONE_SUM_STRICT=1`.  
NOTE: Code reference: `app/services/escrow.py:_ensure_activation_allowed`, `app/services/escrow.py:_validate_canonical_milestone_totals`, `app/services/escrow.py:_log_milestone_sum_mismatch`, `app/services/feature_flags.py:is_milestone_sum_strict_enforced`.
...
##### Schema: PartyRef (party identity)
- `party_type` (string enum) — `BENEFICIARY`, `PROVIDER`, `MERCHANT`.  
- `party_kind` (string enum) — `ON_PLATFORM`, `OFF_PLATFORM`.  
- `user_id` (int, nullable) — required for `ON_PLATFORM` parties.  
- `external_profile_id` (int, nullable) — required for `OFF_PLATFORM` non-merchant parties without `registry_id`.  
- `display_name` (string, nullable) — required for `OFF_PLATFORM` parties.  
- `phone` (string, nullable)  
- `country` (string, nullable) — required for `OFF_PLATFORM` parties.  
- `city` (string, nullable)  
- `address_line` (string, nullable)  
- `registry_id` (string, nullable) — merchant registry identifier when applicable.  
- `pricing_profile_id` (string, nullable)  

OFF_PLATFORM proof submitters must submit proofs using an external proof token (see external token APIs).  
`proof_submitter` defines who can upload proofs; `payout_destination` defines who receives released funds. For canonical v1, these PartyRefs are the source of truth and legacy fields are derived mirrors.  
NOTE: Code reference: `app/schemas/party_ref.py:PartyRef._validate_party_ref`, `app/services/escrow.py:_resolve_party_refs_for_read`, `app/utils/party_ref.py:build_proof_submitter_from_legacy`, `app/utils/party_ref.py:build_payout_destination_from_legacy`, `app/security/external_tokens.py:extract_external_token`.
...
- `selected_merchant_source` (string, nullable; `"registry"` or `"suggestion"`)  
- `selected_merchant_id` (string UUID, nullable)  
- `selected_merchant_display_name` (string, nullable)  
- `selected_merchant_country_code` (string, nullable)  
- `amount_total` (string decimal)  
- `currency` (string)  
- `status` (string enum `EscrowStatus`)  
- `domain` (string enum `EscrowDomain`)  
- `payment_mode` (string enum, deprecated) — derived for compatibility (see below).  
- `payout_destination_type` (string enum) — derived from `payout_destination` when present.  
- `proof_submitter` (object, nullable) — PartyRef describing the required proof submitter.  
- `payout_destination` (object, nullable) — PartyRef describing the payout receiver.  
- `release_conditions_json` (object) — legacy `fraud_config` keys are stripped; proof expectations live on proof requests instead of escrow state.【F:app/services/escrow.py†L199-L217】  
- `deadline_at` (string, ISO 8601 datetime)  

Payment mode derivation (compatibility only):  
- If milestones exist → `MILESTONE`.  
- Else if `payout_destination.party_type == MERCHANT` → `DIRECT_PAY`.  
- Else → `NO_MILESTONE_CONDITIONAL`.  
NOTE: Code reference: `app/utils/escrow_routing.py:derive_payment_mode`.

#### Backward compatibility & migration (legacy = compat only)
- `payment_mode` and `payout_destination_type` remain accepted for legacy clients but are derived from PartyRef + milestones when PartyRef is present.  
  NOTE: Code reference: `app/services/escrow.py:create_escrow`, `app/utils/escrow_routing.py:derive_payment_mode`.
- PartyRef read-repair can backfill missing `proof_submitter_json`/`payout_destination_json` on read when `KOB_PARTYREF_READ_REPAIR=1`.  
  NOTE: Code reference: `app/services/escrow.py:_maybe_read_repair_party_refs`, `app/utils/party_ref.py:build_backfill_proof_submitter_ref`, `app/utils/party_ref.py:build_backfill_payout_destination_ref`.
- Direct Pay endpoints are wrappers around merchant payout routing; they remain for compatibility but do not supersede PartyRef routing.  
  NOTE: Code reference: `app/utils/escrow_routing.py:payout_destination_type_for_escrow`, `app/services/escrow.py:_normalize_direct_pay_release_conditions`.

Examples:
- **Merchant payout with no milestones (legacy Direct Pay)**  
  ```json
  {
    "payment_mode": "DIRECT_PAY",
    "payout_destination": {
      "party_type": "MERCHANT",
      "party_kind": "OFF_PLATFORM",
      "display_name": "Atlas Market",
      "country": "FR"
    },
    "release_conditions": {"requires_proof": true}
  }
  ```
- **Provider payout with milestones**  
  ```json
  {
    "payout_destination": {
      "party_type": "PROVIDER",
      "party_kind": "ON_PLATFORM",
      "user_id": 34
    },
    "release_conditions": {
      "requires_proof": true,
      "milestones": [{"label": "Phase 1", "idx": 1}]
    }
  }
  ```
- **Off-platform submitter using token**  
  ```json
  {
    "proof_submitter": {
...
##### Schema: EscrowMerchantSelectionUpdate (request body)
- `source` (string enum `"registry" | "suggestion"`)  
...
- `ai_summary_text` (string, nullable).【F:app/schemas/proof.py†L128-L169】  
...
- `pricing_summary` (PricingSummaryRead).【F:app/schemas/escrow.py†L200-L236】  

`viewer_context` includes `relation` (`SENDER`, `PROVIDER`, `PARTICIPANT`, `OPS`, `UNKNOWN`), relational flags (`is_sender`, `is_provider`, `is_participant`), `viewer_user_id`, and `allowed_actions` computed from escrow/milestone/proof state:  
- Always includes `VIEW_SUMMARY`, `VIEW_MILESTONES`, `VIEW_PROOFS`.  
- Sender relation may include: `ACTIVATE_ESCROW` (draft + milestones + valid fraud config), `FUND_ESCROW` (funding allowed by invariants), `EDIT_MILESTONES` (draft + no proofs/payments), `EDIT_FRAUD_CONFIG_ESCROW` (always), `EDIT_FRAUD_CONFIG_MILESTONE` (current WAITING milestone with no proof), `MARK_DELIVERED` (escrow FUNDED), `CLIENT_APPROVE` (status in DRAFT/ACTIVE/FUNDED/RELEASABLE and requires_proof is false or any proof submitted), `CLIENT_REJECT` (non-terminal), `CHECK_DEADLINE` (FUNDED + past deadline), `DECIDE_PROOF` (pending proof with sender validator and non-advisor/support review_mode), `REQUEST_ADVISOR_REVIEW` (any pending proof).  
- Provider relation may include: `UPLOAD_PROOF_FILE` when escrow is ACTIVE/FUNDED and a current milestone is submittable; `SUBMIT_PROOF` when no active proof exists for that milestone (`PENDING`/`UNDER_REVIEW`).  
- `OPS`/`UNKNOWN` relations keep view-only actions.【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1837】

##### Endpoint: GET /escrows
**Auth scopes:** sender, provider, support.【F:app/routers/escrow.py†L33-L76】  
...
##### Endpoint: PATCH /escrows/{escrow_id}/merchant-selection
**Auth scopes:** sender  
**Request body:** `EscrowMerchantSelectionUpdate`  
**Response:** `EscrowSummaryRead`
...
```json
{
  ...
  "beneficiary_id": null,
  "beneficiary_profile": null,
  "merchant_registry_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "merchant_suggestion_id": null,
  "selected_merchant_source": "registry",
  "selected_merchant_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "selected_merchant_display_name": "Atlas Market",
  "selected_merchant_country_code": "FR",
  "amount_total": "1500.00",
  "currency": "USD",
  "status": "DRAFT",
  "domain": "private",
  ...
  "payment_provider_display_name": "Stripe"
}
```
...
```json
{
  ...
  "pricing_summary": {
    ...
    "pricing_flags": [],
    "last_computed_at": "2024-07-10T17:30:00Z"
  }
}
```
...
POST | /escrows/{escrow_id}/proof-requests | `create_proof_request` | API key | sender | `ProofRequestCreate` | `ProofRead` (status=REQUESTED) | 201 | MILESTONE_NOT_FOUND, MILESTONE_REQUIRED | 【F:app/routers/escrow.py†L260-L308】【F:app/schemas/proof.py†L140-L214】
...
**Proof request contract (proof-bound expectations)**  
- Proof requests are created on `/escrows/{escrow_id}/proof-requests` with required `format`, `content`, `fraud_context_expectations`, and `proof_requirements`. `document_checks_expectations` is only allowed when `format` is PHOTO/PDF and `content` is INVOICE.【F:app/routers/escrow.py†L260-L308】【F:app/schemas/proof.py†L140-L214】  
- `milestone_idx` is optional: when provided the request is milestone-backed; when omitted the request is escrow-level and only permitted when the escrow has no milestones. A milestone may have multiple proof requests.【F:app/routers/escrow.py†L260-L308】【F:app/services/proofs.py†L3028-L3108】  
- Format/content matrix (validated server-side):【F:app/schemas/proof.py†L119-L214】  

| format | allowed content |
| --- | --- |
| PDF | INVOICE, CONTRACT, OTHER |
| PHOTO | INVOICE, WORK_PROGRESS, PRODUCT, MATERIAL, PROOF_OF_DELIVERY |
| VIDEO | WORK_PROGRESS, PROOF_OF_DELIVERY, PRODUCT, MATERIAL |

- `proof_requirements` is reserved for file constraints (`allowed_file_kinds`, `allowed_mime_types`).【F:app/schemas/proof.py†L120-L214】  
- `POST /proofs` accepts `proof_id` to attach a file submission to an existing request; the proof response includes `requested_format`, `requested_content`, and the expectation blocks so the frontend can render the request details.【F:app/routers/proofs.py†L36-L58】【F:app/schemas/proof.py†L31-L148】  
- Uploads accept photo, PDF, and video MIME types (`video/mp4`, `video/quicktime`, `video/webm`).【F:app/routers/uploads.py†L29-L90】【F:app/config.py†L117-L129】  

**Binary download contract (`GET /files/signed/{token}`)**  
- Requires `key` query param (storage key) alongside the signed token and returns raw bytes (no JSON body). `Content-Type` is derived from the resolved content type or guessed from the storage key; defaults to `application/octet-stream` when unknown.【F:app/routers/uploads.py†L165-L207】  
- When signed token resolution specifies `disposition=attachment`, the response sets `Content-Disposition: attachment; filename="<basename>"` using the `key` basename; otherwise it returns inline content with no `Content-Disposition` header.【F:app/routers/uploads.py†L199-L207】  
...
**Fe3ModeState fields**
- `mode`: `"off" | "shadow" | "on"`.  
- `allowed_modes`: list of allowed strings (currently `["off", "shadow", "on"]`).  
- `updated_at`: ISO timestamp when last changed (nullable on seed).  
- `updated_by`: `{user_id, scope}` for the actor who last updated the mode (nullable).  
- `reason`: operator-provided rationale for the last change (nullable on seed).【F:app/schemas/fe3_mode.py†L8-L37】

**Fe3ModeUpdateRequest fields**
- `mode`: `"off" | "shadow" | "on"` (normalized to lowercase).  
- `reason`: non-empty string required for updates (trimmed).【F:app/schemas/fe3_mode.py†L38-L50】

**Mode semantics (runtime)**
- **OFF**: FE3 comparisons may be skipped or computed for diagnostics, but projections are not applied (no gating, no review blocking).  
- **SHADOW**: FE3 comparisons run and are stored as shadow bundles; projections are non-blocking (no payout blocks/auto-reject).  
- **ON**: FE3 projections apply to review reasons and enforceable hard-fail rules (e.g., merchant enforcement), per projection policy.【F:app/services/fe3_mode.py†L40-L208】【F:app/services/proofs.py†L3125-L3266】【F:app/services/fraud_pipeline.py†L119-L314】

### Sender fraud expectations (proof-bound)
- **Source of truth**: expectations are created per proof request via `/escrows/{escrow_id}/proof-requests` and returned on `/proofs/{proof_id}`; each proof is evaluated against its own attached expectations.【F:app/routers/escrow.py†L260-L308】【F:app/routers/proofs.py†L124-L150】
- **Request validation**:
  - `fraud_context_expectations` is required for every proof request.  
  - `document_checks_expectations` is only allowed when `format` is `PDF` or `PHOTO` **and** `content` is `INVOICE`.  
  - `proof_requirements` is reserved for file constraints (`allowed_file_kinds`, `allowed_mime_types`).【F:app/schemas/proof.py†L120-L214】
- **Expectation schemas**:
  ...
  - `DocumentChecksExpectationUpdate`: `expected_amount`, `expected_currency`, `amount_tolerance{type:percent|absolute,value}`, `expected_merchant_name`, `expected_merchant_country`, `expected_iban`, `expected_vat_number`, `enforce_match`, `require_observed_fields`.【F:app/schemas/fraud_config.py†L68-L96】
- Comparison bundle result contract: each `comparison_bundle.results[]` item includes `status` (`match` | `mismatch` | `unknown` | `skipped`), `severity`, `comparison_type` (`intent` | `reference` | `policy`), `reason_code` (e.g., `observed_missing`, `no_expected_amount`), and a derived `match` boolean (`true` for match, `false` for mismatch, `null` otherwise). The legacy `reason` field remains for compatibility and mirrors `reason_code` by default.【F:app/services/fraud_intent_comparisons.py†L14-L104】
- Projection outputs (mode=ON): comparison results project to `review_reasons` and `fraud_hard_fail` in proof metadata; `review_reasons` are non-blocking in SHADOW and absent in OFF.【F:app/services/fraud_intent_comparisons.py†L861-L918】【F:app/protocol/policies/fraud_projection_policy.py†L1-L65】
...
Method | Path | Handler | Auth | Required scope(s) | Request model | Response model | Success codes | Common error codes | Evidence
---|---|---|---|---|---|---|---|---|---
POST | /merchants/registry | `create_registry` | API key | admin, support | `MerchantRegistryCreate` | `MerchantRegistryRead` | 201 | INSUFFICIENT_SCOPE | 【F:app/routers/merchant_registry.py†L22-L35】【F:app/schemas/merchants_registry.py†L52-L104】【F:app/security/__init__.py†L136-L153】
GET | /merchants/registry | `list_registry` | API key | any authenticated user (scope not enforced) | Query (limit, offset, country_code, q) | `PaginatedResponse[MerchantRegistryListItemRead]` | 200 | NO_API_KEY, UNAUTHORIZED | 【F:app/routers/merchant_registry.py†L38-L74】【F:app/schemas/merchants_registry.py†L15-L49】【F:app/schemas/pagination.py†L6-L21】【F:app/security/__init__.py†L33-L124】
GET | /merchants/registry/{registry_id} | `get_registry` | API key | any authenticated user (scope not enforced) | — | `MerchantRegistryRead` | 200 | MERCHANT_REGISTRY_NOT_FOUND, NO_API_KEY, UNAUTHORIZED | 【F:app/routers/merchant_registry.py†L77-L91】【F:app/schemas/merchants_registry.py†L31-L49】【F:app/security/__init__.py†L33-L124】
...
Method | Path | Handler | Auth | Required scope(s) | Request model | Response model | Success codes | Common error codes | Evidence
---|---|---|---|---|---|---|---|---|---
POST | /sender/payments/{payment_id}/execute | `execute_sender_payment` | API key (authenticated user) | — (escrow sender relation required) | — | `PaymentRead` | 200 | Auth errors | 【F:app/routers/sender_payments.py†L17-L39】【F:app/services/payments.py†L324-L357】【F:app/security/context.py†L60-L97】
...
GET | /payments/{payment_id} | `read_payment` | API key | any authenticated user (escrow relation required) | — | `PaymentRead` | 200 | PAYMENT_NOT_FOUND, NOT_ESCROW_PARTICIPANT | 【F:app/routers/payments.py†L46-L74】【F:app/services/payments.py†L304-L322】
...
**Sender payout execution semantics**
- `/sender/payments/{payment_id}/execute` accepts any authenticated API key but enforces the escrow sender relationship before execution; if the payment is already executed (status in SENT/SETTLED/ERROR/REFUNDED) it returns the current `PaymentRead` with non-OPS redactions (`psp_ref`, `idempotency_key`).【F:app/routers/sender_payments.py†L17-L39】【F:app/services/payments.py†L324-L357】【F:app/security/context.py†L60-L97】【F:app/utils/redaction.py†L343-L401】
...
GET | /sender/dashboard | `get_sender_dashboard` | Not mounted (router not included) | — | Query: limit (default 10, max 50), include_actions | `SenderDashboardRead` | — | — | 【F:app/routers/sender_dashboard.py†L29-L55】【F:app/routers/__init__.py†L33-L63】
GET | /admin/dashboard | `read_admin_dashboard` | Not mounted (router not included) | — | — | `AdminDashboard` | — | — | 【F:app/routers/admin_dashboard.py†L11-L28】【F:app/routers/__init__.py†L33-L63】
GET | /admin/senders | `list_admin_senders` | Not mounted (router not included) | — | Query (limit, offset, q) | `PaginatedResponse[AdminSenderRead]` | — | — | 【F:app/routers/admin_senders.py†L24-L62】【F:app/routers/__init__.py†L33-L63】
...
- **EscrowCreate**: sender/provider ids, beneficiary, amount/currency, release_conditions, deadline, domain, canonical_version.【F:app/schemas/escrow.py†L22-L69】
...
### Advisor
- **AdvisorProfileCreate/Read/Update**: advisor identity, status, ratings, timestamps, plus `managed_country_codes` (sorted ISO alpha-2 codes).【F:app/schemas/advisor.py†L7-L61】
...
ESCROW_MILESTONES_NOT_ALLOWED | 400 | Milestones are not allowed for merchant payout escrows | `error_codes` catalog | 【F:app/utils/error_codes.py†L14-L108】
ESCROW_MILESTONES_SUM_MISMATCH | 422 | Milestone amounts must sum to escrow amount_total | `error_codes` catalog | 【F:app/utils/error_codes.py†L15-L116】
ESCROW_MILESTONES_CURRENCY_MISMATCH | 422 | Milestone currency must match escrow currency | `error_codes` catalog | 【F:app/utils/error_codes.py†L16-L124】
ESCROW_CANONICAL_MISSING_PARTYREF | 422 | Canonical escrow requires proof_submitter + payout_destination | `error_codes` catalog | 【F:app/utils/error_codes.py†L12-L120】
...
DEPRECATED_ENDPOINT | 410 | Deprecated endpoint | `error_codes` catalog | 【F:app/utils/error_codes.py†L41-L167】
...
MILESTONE_ID_REQUIRED_FOR_MILESTONE_ESCROW | 409 | Milestone id is required for milestone escrows | `error_codes` catalog | 【F:app/utils/error_codes.py†L16-L120】
...
PROOF_UPLOADER_MISMATCH_CANONICAL | 403 | Proof uploader does not match canonical submitter | `error_codes` catalog | 【F:app/utils/error_codes.py†L25-L240】
...
- Escrow summary redaction is relationship-based: proofs remove AI scores (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`) for non-OPS viewers and strip payout/pricing metadata for participant relations; advisor views keep limited analytics (`pricing_features`, `price_reference`, `risk_decision`) while still removing payout rails; and payments hide `psp_ref`/`idempotency_key` for non-OPS viewers.【F:app/services/escrow.py†L1683-L1707】【F:app/utils/redaction.py†L203-L401】
- AI summary text exposure is restricted to `SENDER`/OPS contexts (relation-aware) and is nulled for other relations even when proofs are otherwise visible.【F:app/protocol/policies/ai_exposure_policy.py†L90-L132】【F:app/utils/redaction.py†L312-L370】
...
```
Escrow: DRAFT -> ACTIVE -> FUNDED -> RELEASABLE -> RELEASED (funds released; milestone payouts complete at Milestone.PAID)
...
Proof: REQUESTED -> SUBMITTED/PENDING -> APPROVED/REJECTED
      APPROVED -> REJECTED
