...
11. Les preuves/milestones/payouts évoluent via relations d’escrow (viewer_relation), pas via des labels globaux ; se fier aux endpoints, scopes et `viewer_context` du guide.【F:app/security/authz_rel.py†L39-L107】【F:app/schemas/escrow.py†L182-L203】
12. Pour le flux externe, consommer uniquement `/external/escrows/summary` (token-only) et ignorer la variante path `/external/escrows/{escrow_id}` sauf compatibilité legacy.【F:app/routers/external_proofs.py†L318-L389】
13. Traiter les champs redacted comme “non disponibles” : scores AI, `ai_summary_text`, métadonnées sensibles et `psp_ref`/`idempotency_key` peuvent être nulls selon la relation d’escrow et le mode ops ; ne jamais inférer ces valeurs côté UI.【F:app/utils/redaction.py†L203-L401】【F:app/protocol/policies/ai_exposure_policy.py†L90-L132】【F:app/schemas/payment.py†L8-L33】
14. Toujours afficher un message clair sur 401/403 et purger le token si 401 (cf. FRONTEND_API_GUIDE, Auth section).
15. Pas de retry en boucle sur 500 uploads : afficher échec, proposer réupload avec même fichier/sha pour tracer (cf. section 6.2).
...
- Adapter l’UI selon scope + relation d’escrow (`viewer_context.relation`) retournée par `/escrows/{id}/summary`, plutôt que des labels globaux.【F:app/services/escrow.py†L1683-L1715】【F:app/schemas/escrow.py†L182-L203】
- Respecter la redaction relationnelle : certains champs (scores AI, métadonnées sensibles, `psp_ref`, `idempotency_key`) sont nullifiés hors contexte ops/sender, donc la UI doit supporter des valeurs nulles sans déduction client-side.【F:app/utils/redaction.py†L203-L401】【F:app/protocol/policies/ai_exposure_policy.py†L90-L132】
