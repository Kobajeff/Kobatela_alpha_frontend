...
- Proof requests are created in `REQUESTED` status; submitted proofs transition to `PENDING` for review. 【F:app/services/proofs.py†L3717-L4140】
...
| PATCH | `/escrows/{escrow_id}/merchant-selection` | Select merchant for Direct Pay | sender | `EscrowMerchantSelectionUpdate` → `EscrowSummaryRead` | Draft-only; supports registry/suggestion sources and returns snapshot fields. | 【F:app/routers/escrow.py†L216-L235】【F:app/schemas/escrow.py†L143-L163】 |
| POST | `/escrows/{escrow_id}/direct-pay/release` | Release Direct Pay escrow | sender | — → `PaymentRead` | Requires approved escrow-level proof + selected merchant; executes merchant payout. | 【F:app/routers/escrow.py†L240-L264】 |
...
- `payment_mode` (string enum `"MILESTONE" | "DIRECT_PAY" | "NO_MILESTONE_CONDITIONAL"`, deprecated UI template; read-only for canonical v1)  
- `payout_destination_type` (string enum `"BENEFICIARY_PROVIDER" | "MERCHANT"`, nullable; derived from `payout_destination` when present, read-only for canonical v1)  
- `proof_submitter` (PartyRef, nullable)  
- `payout_destination` (PartyRef, nullable)  
- `canonical_version` (string, nullable; `"v1"` opt-in when enabled)  

##### Canonical v1 vs Legacy (compat-only)
- **Canonical v1 (recommended)**: opt-in via `canonical_version="v1"` when `KOB_CANONICAL_ESCROW_V1=1`. Canonical v1 requires `proof_submitter` + `payout_destination` and derives routing from PartyRef + milestones (legacy `payment_mode`/`payout_destination_type` inputs are ignored).  
...
- **Legacy (compatibility only)**: PartyRef is optional unless `KOB_ENFORCE_PARTYREF_ON_LEGACY_CREATE=1`. When PartyRef is provided, routing derives from PartyRef + milestones and can override legacy payout hints.  
...
##### PartyRef (shared party identity)
- `party_type` (`BENEFICIARY`, `PROVIDER`, `MERCHANT`)  
- `party_kind` (`ON_PLATFORM`, `OFF_PLATFORM`)  
- `user_id` (int, required for `ON_PLATFORM`)  
- `external_profile_id` (int, required for `OFF_PLATFORM` non-merchant parties without `registry_id`)  
- `display_name`/`country` (required for `OFF_PLATFORM`)  
- `registry_id` (string, nullable; merchant registry id)  

**Submitter vs payout destination responsibilities**
- `proof_submitter` controls **who can upload/submit proofs** (OFF_PLATFORM submitters must use external proof tokens).  
- `payout_destination` controls **who receives released funds**; merchant destinations drive Direct Pay routing.  
NOTE: Code reference: `app/schemas/party_ref.py:PartyRef._validate_party_ref`, `app/services/escrow.py:_resolve_party_refs_for_read`, `app/security/external_tokens.py:extract_external_token`, `app/utils/escrow_routing.py:payout_destination_type_for_escrow`.

**OFF_PLATFORM proof submitters** must use external proof tokens (`Authorization: Bearer <external_token>`). See section 8.2 for token issuance and external flows.【F:app/security/external_tokens.py†L1-L25】【F:app/routers/external_proof_tokens.py†L39-L172】  

##### Canonical v1 create examples (PartyRef-first)
**(a) No milestones + escrow-level proof requests**
```json
{
  "client_id": 12,
  "provider_user_id": 34,
...
```
Then create escrow-level proof requests with `POST /escrows/{escrow_id}/proof-requests` (no `milestone_id`).  

**(b) Two milestones + per-milestone proof requests**
```json
{
  "client_id": 12,
  "provider_user_id": 34,
...
```
Create milestones via `/escrows/{escrow_id}/milestones`, then create proof requests for each milestone (`milestone_id` required).  

**(c) Merchant payout template (Direct Pay via PartyRef)**
```json
{
  "client_id": 12,
  "provider_user_id": 34,
  ...
  "payout_destination": {
    "party_type": "MERCHANT",
    "party_kind": "ON_PLATFORM",
    "registry_id": "3e56d2b4-6fa3-4c82-9f85-18e9c1f6e70a"
  }
}
```
This keeps direct-pay endpoints working but routes payouts based on the PartyRef merchant destination.  

**External submitters (OFF_PLATFORM)**: set `proof_submitter.party_kind=OFF_PLATFORM` and issue an external proof token (`POST /sender/external-proof-tokens`) for uploads.  

##### Backward compatibility & migration (legacy = compat only)
- Legacy fields (`payment_mode`, `payout_destination_type`) remain accepted for older clients but are derived from PartyRef + milestones when PartyRef is present.  
  NOTE: Code reference: `app/services/escrow.py:create_escrow`, `app/utils/escrow_routing.py:derive_payment_mode`.
- PartyRef read-repair can backfill missing JSON PartyRef fields on read when `KOB_PARTYREF_READ_REPAIR=1`.  
...
- `selected_merchant_source` (string, nullable) — `"registry"` or `"suggestion"` when a selection is saved.  
- `selected_merchant_id` (string UUID, nullable) — selected merchant id snapshot.  
- `selected_merchant_display_name` (string, nullable) — selected merchant name snapshot.  
- `selected_merchant_country_code` (string, nullable) — selected merchant country snapshot.  
- `amount_total` (string decimal)  
- `currency` (string)  
- `status` (string enum `EscrowStatus`)  
- `domain` (string enum `EscrowDomain`)  
- `payment_mode` (string enum `"MILESTONE" | "DIRECT_PAY" | "NO_MILESTONE_CONDITIONAL"`, derived for compatibility)  
- `payout_destination_type` (string enum)  
- `proof_submitter` (PartyRef, nullable)  
- `payout_destination` (PartyRef, nullable)  
- `release_conditions_json` (object) — legacy `fraud_config` keys are stripped; proof expectations are proof-request-bound. 【F:app/services/escrow.py†L199-L217】  
- `deadline_at` (string, ISO 8601 datetime)  
...
- `selected_merchant_*` fields are null until a selection is persisted via `/escrows/{escrow_id}/merchant-selection`.【F:app/schemas/escrow.py†L55-L79】  
...
- `payment_mode` (string enum `"MILESTONE" | "DIRECT_PAY" | "NO_MILESTONE_CONDITIONAL"`)  
- `provider_user_id` (int, nullable) — null until an on-platform provider is linked.  
- `provider_id` (int, nullable) — alias of `provider_user_id` (same nullability).  
- `beneficiary_id` (int, nullable) — null when no beneficiary profile is attached.  
...
##### EscrowMerchantSelectionUpdate (request body)
- `source` (`"registry"` or `"suggestion"`)  
...
- `allowed_actions` is the backend source of truth for CTAs and always includes `VIEW_SUMMARY`, `VIEW_MILESTONES`, `VIEW_PROOFS`. The escrow sender relation may additionally receive `ACTIVATE_ESCROW` (draft + milestones), `FUND_ESCROW` (funding allowed by invariants), `EDIT_MILESTONES` (draft with no proofs/payments), `MARK_DELIVERED` (escrow FUNDED), `CLIENT_APPROVE` (status in DRAFT/ACTIVE/FUNDED/RELEASABLE and proof gating satisfied: legacy uses `release_conditions.requires_proof` + proof submitted; canonical v1 or global gating requires approved proof), `CLIENT_REJECT` (non-terminal), `CHECK_DEADLINE` (FUNDED + past deadline), `DECIDE_PROOF` (pending proof with sender validator and non-advisor/support review_mode), and `REQUEST_ADVISOR_REVIEW` (any pending proof). The escrow provider relation may receive `UPLOAD_PROOF_FILE` when escrow status is ACTIVE/FUNDED and a current milestone is submittable, and `SUBMIT_PROOF` when no active proof exists for that milestone (`PENDING`/`UNDER_REVIEW`). `OPS`/`UNKNOWN` relations keep only the view actions.【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1837】

**Error handling rules (proof/milestone gating)**
- `PROOF_REQUIRED` (400): client-approve/release flows require at least one submitted (or approved) proof when proof gating is active.  
  NOTE: Code reference: `app/services/escrow.py:_ensure_release_conditions`.
- `ESCROW_PROOF_COHERENCE_MISSING_MILESTONE_PROOF_REQUESTS` / `ESCROW_PROOF_COHERENCE_MISSING_ESCROW_LEVEL_PROOF_REQUESTS` (409): PR1/PR2 are violated.  
  NOTE: Code reference: `app/services/proof_coherence.py:validate_proof_coherence`.
- `MILESTONE_RELEASE_AMOUNT_MISMATCH` (409) or `MILESTONE_ID_REQUIRED_FOR_MILESTONE_ESCROW` (409): milestone releases must match a specific milestone amount and include the milestone id.  
  NOTE: Code reference: `app/services/payments.py:_assert_payout_preconditions`.

**UI recommendations**
- Do not assume "visible" implies "allowed": always gate CTAs based on `viewer_context.allowed_actions`, and re-check after state-changing calls.  
  NOTE: Code reference: `app/protocol/policies/escrow_allowed_actions.py:compute_allowed_actions`.
...
### 4.3 Proof Upload + Requests + Submit + List + Decision
| Method | Path | Summary | Scope | Request → Response | Notes | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
...
| POST | `/escrows/{escrow_id}/proof-requests` | Create proof request | sender | `ProofRequestCreate` → `ProofRead` | Creates a REQUESTED proof with expectations stored on the proof. | 【F:app/routers/escrow.py†L260-L308】【F:app/schemas/proof.py†L140-L214】 |
| POST | `/proofs` | Submit proof metadata | sender/provider/support/admin | `ProofCreate` → `ProofRead` | Returns proof payload (normalized numeric fields). Accepts `proof_id` to attach a submission to an existing request. | 【F:app/routers/proofs.py†L36-L58】【F:app/services/proofs.py†L3717-L4140】【F:app/schemas/proof.py†L31-L157】 |
...
**Proof request expectations (frontend contract)**  
- Proof expectations live on the proof request itself (via `/escrows/{escrow_id}/proof-requests`) and are returned on `GET /proofs/{proof_id}` for UI rendering.【F:app/routers/escrow.py†L260-L308】【F:app/routers/proofs.py†L124-L150】  
- `milestone_idx` is optional. When provided, the request is scoped to that milestone; when omitted, the request is escrow-level and only allowed for escrows without milestones. A milestone can have multiple proof requests.【F:app/routers/escrow.py†L260-L308】【F:app/services/proofs.py†L3028-L3108】  
- `document_checks_expectations` is only accepted for INVOICE content on PHOTO or PDF formats; VIDEO is fraud-context-only for now. `proof_requirements` only carries file constraints (`allowed_file_kinds`, `allowed_mime_types`).【F:app/schemas/proof.py†L120-L214】  
...
- `storage_url`/`sha256` are null for proof requests that have not been submitted yet.  
...
- `requested_format`, `requested_content`, and expectation blocks (`fraud_context_expectations`, `document_checks_expectations`, `proof_requirements`) are surfaced so the UI can render requested proof details.  
...
Direct Pay is a **merchant payout pattern**, not a parallel escrow model. UI should treat it as a PartyRef merchant payout destination, with endpoints acting as wrappers around the same release rules.  
...
`AdvisorProfileRead.managed_country_codes` returns a sorted list of ISO alpha-2 country codes (uppercase) managed by the advisor; defaults to `[]` when none are assigned.【F:app/schemas/advisor.py†L23-L45】【F:app/models/advisor.py†L38-L74】
...
- `Fe3ModeState`: `mode` (`off`/`shadow`/`on`), `allowed_modes[]`, `updated_at`, `updated_by{user_id,scope}`, `reason`.  
- `Fe3ModeUpdateRequest`: `mode` (`off`/`shadow`/`on`), `reason` (required).【F:app/schemas/fe3_mode.py†L8-L50】
- Mode semantics: **OFF** (no projection), **SHADOW** (compute + shadow bundle only), **ON** (projection applies to review reasons and enforceable hard-fail rules).【F:app/services/fe3_mode.py†L40-L208】【F:app/services/proofs.py†L3125-L3266】
...
- Runtime behavior: expectations are attached per proof request; computed FE3 outputs (`fraud_features_*`, `comparison_bundle*`, `review_reasons`, `fraud_flags`, `fraud_hard_fail`, `risk_decision`) are stripped from requests; fraud_context accepts optional `human_hints`/`policy_hooks` and alias `fraud_context_intent`.【F:app/services/proofs.py†L197-L210】【F:app/services/proofs.py†L3125-L3466】【F:tests/test_proof_metadata_safety.py†L40-L77】
- Comparison bundle results: `comparison_bundle.results[]` includes `status` (`match` | `mismatch` | `unknown` | `skipped`), `severity`, `comparison_type` (`intent` | `reference` | `policy`), `reason_code`, and derived `match` (boolean or null for unknown/skipped). The legacy `reason` mirrors `reason_code` for compatibility.【F:app/services/fraud_intent_comparisons.py†L14-L104】
- Projection outputs (mode=ON): comparison results project to `review_reasons` and `fraud_hard_fail`; SHADOW keeps bundles in `comparison_bundle_shadow` without blocking. 【F:app/services/fraud_intent_comparisons.py†L861-L918】【F:app/protocol/policies/fraud_projection_policy.py†L1-L65】
...
- Direct Pay is a UI template; merchant payouts are determined by `payout_destination.party_type == "MERCHANT"`.
- You may still send `payment_mode="DIRECT_PAY"` for legacy clients, but routing uses PartyRef + milestone presence.
- Merchant payout escrows require a merchant selection via `PATCH /escrows/{escrow_id}/merchant-selection`.
- Milestones with merchant payouts are guarded by `KOB_ALLOW_MERCHANT_MILESTONES` (default off); when off, the API returns `ESCROW_MILESTONES_NOT_ALLOWED`.
- Sender selects the merchant via `PATCH /escrows/{escrow_id}/merchant-selection`.
- Provider submits escrow-level proofs via `POST /proofs` without `milestone_idx`.
- Sender approves the proof via `POST /proofs/{proof_id}/decision`, then triggers release via `POST /escrows/{escrow_id}/direct-pay/release`.

**No-milestone conditional escrow (provider payout)**
- No-milestone conditional escrows are identified by having zero milestones and a non-merchant payout destination.
- `payment_mode="NO_MILESTONE_CONDITIONAL"` remains accepted for backward compatibility.
- Provider submits escrow-level proofs via `POST /proofs` without `milestone_idx`; sender approves via `POST /proofs/{proof_id}/decision`.
- Releases follow the standard provider payout path (no `/direct-pay/release`).
...
```
For Direct Pay escrows, submit proofs without `milestone_idx` to create escrow-level proofs. 【F:app/schemas/proof.py†L133-L191】
