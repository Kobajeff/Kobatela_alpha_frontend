...
**Constraints:**
* Sender identity is derived from the API key and must pass `can_initiate_escrow`; spoofing `sender_user_id` is blocked with 403 `SENDER_SPOOF_BLOCKED` and ineligible sender roles receive 403 `FORBIDDEN`.【F:app/services/escrow.py†L1275-L1306】
...
`sender_user_id` in escrow payload (sender UI flow) | Sender derived from API key; mismatches return 403 `SENDER_SPOOF_BLOCKED`. | Escrow sender spoof check【F:app/services/escrow.py†L1275-L1302】
...
## Change log
### Substitutions
* **Before:** “Sender identity is derived from the API key; spoofing `sender_user_id` is blocked with 403 `SENDER_SPOOF_BLOCKED` when mismatched.”

  **After:** “Sender identity is derived from the API key and must pass `can_initiate_escrow`; spoofing `sender_user_id` is blocked with 403 `SENDER_SPOOF_BLOCKED` and ineligible sender roles receive 403 `FORBIDDEN`.”

  **Reason:** Escrow creation also enforces initiator role eligibility and returns 403 `FORBIDDEN` for disallowed roles; the previous line omitted this guard.

  **Backend source:** `create_escrow_for_api_key` eligibility and spoof checks.【F:app/services/escrow.py†L1275-L1306】

* **Before:** Forbidden-fields evidence cited `app/services/escrow.py†L1152-L1186` for `sender_user_id` spoofing.

  **After:** Evidence now cites the actual spoof guard at `app/services/escrow.py†L1275-L1302`.

  **Reason:** The previous citation pointed to milestone code, not the sender spoof validation.

  **Backend source:** `create_escrow_for_api_key` sender spoof validation.【F:app/services/escrow.py†L1275-L1302】

### Additions
* Added this change log section to satisfy DOCUMENT SYNC SAFE MODE deliverable requirements.
