...
- Relation-based ABAC context is computed from escrow linkage (`sender_user_id`, `provider_user_id`, optional participants) and ops/advisor flags; the resulting `viewer_context.relation` (`SENDER`/`PROVIDER`/`PARTICIPANT`/`OPS`/`UNKNOWN`) and `allowed_actions` are used in escrow summaries to drive permitted actions per relation.【F:app/security/authz_rel.py†L39-L75】【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1807】
...
Method | Path | Handler | Auth | Required scope(s) | Request model | Response model | Success codes | Common error codes | Evidence
---|---|---|---|---|---|---|---|---|---
POST | /sender/payments/{payment_id}/execute | `execute_sender_payment` | API key | sender | — | `PaymentRead` | 200 | Auth errors | 【F:app/routers/sender_payments.py†L17-L39】【F:app/services/payments.py†L324-L357】
...
- `/sender/payments/{payment_id}/execute` enforces escrow-sender relationship checks and returns a redacted `PaymentRead` when the payment has already been executed (status in SENT/SETTLED/ERROR/REFUNDED).【F:app/services/payments.py†L324-L357】【F:app/utils/redaction.py†L343-L401】
...
- Escrow summary redaction is relationship-based: milestones redact `proof_requirements` for `PROVIDER`/`PARTICIPANT` viewers; proofs remove AI scores (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`) and payout/pricing metadata for any non-OPS relation; and payments hide `psp_ref`/`idempotency_key` for non-OPS relations.【F:app/services/escrow.py†L1683-L1707】【F:app/utils/redaction.py†L203-L401】
