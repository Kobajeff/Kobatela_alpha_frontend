...
- Scope: tokens are single-use, escrow + milestone + beneficiary bound; `consume_external_proof_token` enforces purpose/expiry, rejects reused tokens with 409.【F:app/services/external_proof_tokens.py†L62-L148】
- TTL: default 72h; min 5 minutes; max 7 days. Custom TTL outside bounds returns 422 (`TOKEN_EXPIRY_TOO_SHORT`/`TOKEN_EXPIRY_TOO_LONG`).【F:app/services/external_proof_tokens.py†L40-L118】
- Issuers: sender (must own escrow), support, admin. Beneficiary_id must match escrow; otherwise 403 (`BENEFICIARY_MISMATCH`).【F:app/services/external_proof_tokens.py†L62-L118】
...
- Recommended polling: start at 2s interval, exponential backoff to 15s max with jitter; stop when `terminal` is true or after 5 minutes total wait to avoid hammering (UI responsibility).
...
| Token expired/used/wrong purpose | 401/409 | FORBIDDEN / TOKEN_ALREADY_USED | Token consumption【F:app/services/external_proof_tokens.py†L78-L148】 |
