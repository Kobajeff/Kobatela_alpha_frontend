...
- Proof AI scores (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`) are cleared for any non-OPS relation; `ai_summary_text` is exposed only for `SENDER`/`OPS` relations.  
- Proof metadata is stripped of payout rails and fraud/pricing internals for `SENDER`/`PROVIDER`/`PARTICIPANT` relations; advisors receive a partially filtered subset (no payout rails, limited analytics).  
- Payments redact `psp_ref` and `idempotency_key` for non-OPS relations.  
