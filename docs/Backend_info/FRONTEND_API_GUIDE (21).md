...
Typical escrow lifecycle for UI flows: `DRAFT → ACTIVE → FUNDED → RELEASABLE → RELEASED` (funds released), with cancellation/refund transitions enforced by the escrow state machine; milestone payout completion is represented by the Milestone `PAID` status instead of the escrow status name.【F:app/services/state_machines.py†L53-L116】
...
| GET | `/provider/inbox/escrows` | Provider inbox (relationship-based) | authenticated user | Query (`limit`, `offset`) → `ProviderInboxResponse` | Returns provider-focused inbox with proof requirements already redacted. | 【F:app/routers/provider_inbox.py†L16-L36】【F:app/schemas/provider_inbox.py†L8-L29】 |
...
`viewer_context` details (relationship-based access, not global roles):
- `relation` is one of `SENDER`, `PROVIDER`, `PARTICIPANT`, `OPS`, `UNKNOWN`, derived from escrow links or ops status.  
- `is_sender`, `is_provider`, `is_participant`, `viewer_user_id` are always present to drive UI gating.  
- `allowed_actions` is the backend source of truth for CTAs and always includes `VIEW_SUMMARY`, `VIEW_MILESTONES`, `VIEW_PROOFS`. Escrow-sender relations may additionally receive `ACTIVATE_ESCROW` (draft + milestones + valid fraud config), `FUND_ESCROW` (funding allowed by state machine), `EDIT_MILESTONES` (draft with no proofs/payments), `EDIT_FRAUD_CONFIG_ESCROW` (always for sender relation), `EDIT_FRAUD_CONFIG_MILESTONE` (open waiting milestone without proof), `MARK_DELIVERED` (escrow FUNDED), `CLIENT_APPROVE` (release condition satisfied), `CLIENT_REJECT` (non-terminal), `CHECK_DEADLINE` (FUNDED + past deadline), `DECIDE_PROOF` (pending proof with sender validator), and `REQUEST_ADVISOR_REVIEW` (any pending proof). Escrow-provider relations may receive `UPLOAD_PROOF_FILE` and `SUBMIT_PROOF` when the escrow is ACTIVE/FUNDED and the current milestone is submittable with no active proof. `OPS`/`UNKNOWN` relations keep only the view actions.【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1683-L1715】
...
- Proof AI scores (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`) are cleared for any non-ops viewer; `ai_summary_text` is exposed only for `SENDER`/`OPS` contexts.  
- Proof metadata is stripped of payout rails and fraud/pricing internals for non-ops viewers; advisors receive a partially filtered subset (no payout rails, no internal analytics).  
- Payments redact `psp_ref` and `idempotency_key` for non-ops viewers.  
...
| GET | `/files/signed/{token}` | **NOT FOR UI** signed download | None (signed token) | — → raw file | Token + storage key required; returns bytes (no JSON) with `Content-Type` from stored content or key extension (fallback `application/octet-stream`) and `Content-Disposition` only when attachment is requested. | 【F:app/routers/uploads.py†L165-L207】 |
...
- `ai_*` fields (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`, `ai_flags`, `ai_checked_at`, `ai_reviewed_at`, `ai_reviewed_by`) are nullable until the AI pipeline runs and are also nulled for non-ops viewers.  
- `ai_summary_text` is nullable and only present for `SENDER`/`OPS` contexts.  
...
| Method | Path | Summary | Scope | Request → Response | UI Label | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| POST | `/sender/payments/{payment_id}/execute` | Execute payout as sender | sender | — → `PaymentRead` | Requires escrow-sender relation; returns redacted `PaymentRead` if already executed. | 【F:app/routers/sender_payments.py†L17-L39】【F:app/services/payments.py†L305-L347】 |
