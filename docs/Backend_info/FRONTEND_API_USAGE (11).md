...
- Le flux lié à la relation d’escrow `SENDER` doit piloter les CTA via `viewer_context.allowed_actions` au lieu de déduire les actions côté UI ; ces actions sont calculées par le backend selon la relation d’escrow (`SENDER`/`PROVIDER`/`PARTICIPANT`/`OPS`/`UNKNOWN`) et l’état courant.【F:app/schemas/escrow.py†L182-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1837】
...
11. `viewer_context` expose la relation d’escrow (`relation`), l’identité de vue (`viewer_user_id`) et les capacités (`allowed_actions`) ; `allowed_actions` inclut toujours `VIEW_SUMMARY`, `VIEW_MILESTONES`, `VIEW_PROOFS` et n’ajoute les actions métier que si la relation d’escrow et l’état le permettent (ex: `ACTIVATE_ESCROW`, `FUND_ESCROW`, `UPLOAD_PROOF_FILE`, `SUBMIT_PROOF`). Traiter cette structure comme la source de vérité d’autorisation relationnelle côté UI.【F:app/schemas/escrow.py†L154-L203】【F:app/protocol/policies/escrow_allowed_actions.py†L15-L204】【F:app/services/escrow.py†L1760-L1837】
...
14. Traiter les champs redacted comme “non disponibles” : les relations non-OPS ne reçoivent pas les scores AI (`ai_score`, `ai_score_ml`, `ai_risk_level_ml`, `ai_explanation`), `ai_summary_text` n’est visible que pour la relation `SENDER`/OPS, les métadonnées sont filtrées des rails payout/pricing (les vues advisor gardent `pricing_features`, `price_reference`, `risk_decision`), et `psp_ref`/`idempotency_key` sont nullifiés hors OPS ; ne jamais inférer ces valeurs côté UI.【F:app/utils/redaction.py†L203-L401】【F:app/protocol/policies/ai_exposure_policy.py†L90-L132】【F:app/schemas/payment.py†L8-L33】
15. Les téléchargements signés sont binaires : `GET /files/signed/{token}` exige `key` en query, renvoie des octets sans JSON, `Content-Type` est résolu via content_type ou l’extension, et `Content-Disposition` est présent uniquement si la résolution signée marque `attachment`.【F:app/routers/uploads.py†L165-L207】
...
| Exécuter payout (sender) | POST /sender/payments/{id}/execute | Non | Invalider payment + summary; démarrer polling payout | Si paiement déjà exécuté, le backend renvoie le `PaymentRead` actuel (status SENT/SETTLED/ERROR/REFUNDED) : désactiver le bouton et rafraîchir. | 【F:app/routers/sender_payments.py†L12-L39】【F:app/services/payments.py†L324-L357】【F:app/security/context.py†L60-L97】 |
