        ...
        <tbody className="divide-y divide-slate-200">
          {escrows.map((escrow) => (
            <tr key={escrow.id} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-3 text-sm text-slate-700">{escrow.status}</td>
              <td className="px-4 py-3 text-sm text-slate-700">
                {escrow.amount} {escrow.currency}
              </td>
              <td className="px-4 py-3 text-sm text-slate-500">{new Date(escrow.created_at).toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
